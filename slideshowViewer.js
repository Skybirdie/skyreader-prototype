...
function stopForMediaManager() {
    ...
    function startSelectedAudio() { stopAudio(); audioCompleted = false; if (!current) { return; } /* ------------------------------------------------------- ORIGINAL ITEM AUDIO The shared item carries its audio URL in: current.audio Keep that URL attached to the actual slideshow <audio> element even when the browser refuses autoplay on initial Share Mode startup. ------------------------------------------------------- */ if (audioMode === "original" && current.audio && audio) { let audioUrl = String(current.audio).trim(); /* * Some content sources can provide a Markdown-style * link rather than the bare URL. Extract the URL if * necessary. */ const markdownMatch = audioUrl.match( /^\[([^\]]+)\]\((https?:\/\/[^)]+)\)$/ ); if (markdownMatch) { audioUrl = markdownMatch[2]; } /* * Remove accidental surrounding quotes. */ audioUrl = audioUrl .replace(/^["']+|["']+$/g, "") .trim(); if (!audioUrl) { console.warn( "[SlideshowViewer] Original audio was selected, " + "but the audio URL is empty." ); return; } console.log( "[SlideshowViewer] Loading original audio:", audioUrl ); audio.src = audioUrl; audio.muted = muted; audio.preload = "auto"; /* * Keep the audio element connected to the source even * if autoplay is blocked. */ audio.load(); /* * If Share Mode opened the item without a user gesture, * the browser may reject this play(). That is not a * failure of the audio connection. */ if (playing) { const playPromise = audio.play(); if (playPromise && typeof playPromise.catch === "function") { playPromise.catch(error => { console.warn( "[SlideshowViewer] Original audio autoplay " + "was blocked or failed:", error ); /* * Do not remove audio.src here. * The audio remains loaded and can be started * by the user's Play control. */ }); } } return; } /* ------------------------------------------------------- MUSIC ------------------------------------------------------- */ if ( audioMode === "music" && (selectedMusicTrack || MUSIC_LIBRARY.length) ) { const track = selectedMusicTrack || MUSIC_LIBRARY[0]; musicAudio = new Audio(track.url); musicAudio.loop = false; musicAudio.muted = muted; musicAudio.addEventListener("ended", () => { audioCompleted = true; if ( playing && slideCount() && index >= slideCount() - 1 ) { finish(); } }); if (playing) { const playPromise = musicAudio.play(); if (playPromise && typeof playPromise.catch === "function") { playPromise.catch(error => { console.warn( "[SlideshowViewer] Music autoplay was blocked " + "or failed:", error ); }); } } } } function setAudioMode(mode) { const select = document.getElementById("slideshowAudioMode"); const requested = ["none", "original", "effects", "music"].includes(mode) ? mode : "none"; /* * Original sound is available only when this item actually * contains an audio URL. */ if ( requested === "original" && !current?.audio ) { audioMode = "none"; } else { audioMode = requested; } /* * Keep the visible selector synchronized with the * actual mode. */ if (select) { select.value = audioMode; const original = select.querySelector( 'option[value="original"]' ); if (original) { original.disabled = !current?.audio; } const music = select.querySelector( 'option[value="music"]' ); if (music) { music.disabled = !MUSIC_LIBRARY.length; } } /* * Start the selected audio source. * * For Share Mode this is now also responsible for * attaching current.audio to the actual <audio> element. */ startSelectedAudio(); setStatus( audioMode === "original" ? "Original sound" : audioMode === "music" ? "Music" : audioMode === "effects" ? "Page turn effects" : "No sound" ); }
