...
dom.libraryButton.onclick=()=>{

    /* The Library control is an escape/change-book action while
       a reader is open. Closing here lets the user immediately
       choose another publication without turning a page first. */
...
/*
 * The navigation buttons are intentionally anchored to #viewerArea, not
 * to the rendered book.  This keeps their spacing and symmetry constant
 * when a portrait/landscape PDF changes size or when the book is centered
 * differently during initialization.
 */
...
const left=viewerRect.left-toolbarRect.left;
const top=viewerRect.top-toolbarRect.top;
const width=viewerRect.width;
const height=viewerRect.height;
const centerX=left+width/2;
const centerY=top+height/2;

function place(buttonEl,x,y,angle){
    buttonEl.style.left=`${x}px`;
    buttonEl.style.right="auto";
    buttonEl.style.top=`${y}px`;
    buttonEl.style.bottom="auto";
    buttonEl.style.transform=`rotate(${angle}deg)`;
}

/* Both arrows use the same rotation so their directions remain opposite.
   The default SVGs point left (previous) and right (next):
     0°   previous ←   next →
     90°  previous ↑   next ↓
     180° previous →   next ←
     270° previous ↓   next ↑
 */
/* Keep every button fully inside the viewer rectangle.  The button's
 * position is measured from viewerArea itself, never from the PDF page.
 * This also means the 180°/270° states cannot drift beyond the edge. */
const inset=Math.max(6,gap);
const leftInside=left+inset;
const rightInside=left+width-buttonWidth-inset;
const topInside=top+inset;
const bottomInside=top+height-buttonHeight-inset;

if(step===0){
    place(previous,leftInside,centerY-buttonHeight/2,0);
    place(next,rightInside,centerY-buttonHeight/2,0);
}else if(step===90){
    place(previous,centerX-buttonWidth/2,topInside,90);
    place(next,centerX-buttonWidth/2,bottomInside,90);
}else if(step===180){
    place(previous,rightInside,centerY-buttonHeight/2,180);
    place(next,leftInside,centerY-buttonHeight/2,180);
}else{
    place(previous,centerX-buttonWidth/2,bottomInside,270);
    place(next,centerX-buttonWidth/2,topInside,270);
}
